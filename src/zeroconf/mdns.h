...
typedef ssize_t mdns_ssize_t;
    ...
    mdns_announce_multicast( int sock, void* buffer, size_t capacity, mdns_record_t answer,
        ...
        size_t additional_count, uint32_t ttl );
        ...
        size_t size_rhs, size_t* ofs_rhs )
    {
        ...
        do
        {
            ...
            if ( ( lhs_substr.offset == MDNS_INVALID_POS ) || ( rhs_substr.offset == MDNS_INVALID_POS ) || ( counter++ > MDNS_MAX_SUBSTRINGS ) )
                return 0;
        ...
        size_t records, mdns_record_callback_fn callback, void* user_data )
    {
        ...
        for ( size_t i = 0; i < records; ++i )
        {
            ...
            *offset += 10;
            ...
            {
                ++parsed;
                if ( callback && callback( sock, from, addrlen, type, query_id, rtype, rclass, ttl, buffer, size, name_offset, name_length, *offset, length, user_data ) )
                    break;
            }
	...
	*/
        ...
        for ( i = 0; i < answer_rrs; ++i )
        {
            ...
            if ( is_answer )
            {
                ...
                if ( callback && callback( sock, saddr, addrlen, MDNS_ENTRYTYPE_ANSWER, query_id, rtype, rclass, ttl, buffer, data_size, name_offset, name_length, ofs, length, user_data ) )
                    return records;
            }
	...
	*/
        ...
        for ( int iquestion = 0; iquestion < questions; ++iquestion )
        {
            ...
            ++parsed;
            if ( callback && callback( sock, saddr, addrlen, MDNS_ENTRYTYPE_QUESTION, query_id, rtype, rclass, 0, buffer, data_size, question_offset, length, question_offset, length, user_data ) )
                break;
        }
    ...
    mdns_answer_multicast_rclass( int sock, void* buffer, size_t capacity, uint16_t rclass,
        ...
        mdns_record_t* additional, size_t additional_count, uint32_t ttl = 60 )
    {
        if ( capacity < ( sizeof( struct mdns_header_t ) + 32 + 4 ) )
            return -1;
        ...
        mdns_record_t* additional, size_t additional_count, uint32_t ttl )
    {
        ...
        return mdns_answer_multicast_rclass( sock, buffer, capacity, rclass, answer, authority,
            authority_count, additional, additional_count, ttl );
    }
